...
# Video process ke dauran Telegram screenshot monitor
async def live_screenshot_monitor(page, machine_id, interval=20):
    shot_count = 1
    while True:
        try:
            ...
            send_telegram_photo(shot_path, f"🎬 Machine {machine_id} Video Live Status #{shot_count}")
            shot_count += 1
...
async def process_image_to_video(page, image_path, image_num, motion_prompt):
    ...
    # 2. Fill Video & Voice Prompt with Multiple Fallback Locators
    prompt_input = None
    selectors = [
        "textarea",
    ...
    if not prompt_input:
        prompt_input = page.get_by_placeholder("Enter a prompt to generate a video...").first
...
async def main():
    ...
    if not assigned_images:
        print(f"⚠️ Machine {machine_id} has no assigned videos. Exiting.")
        return
    ...
    async with async_playwright() as p:
        ...
        # Start live Telegram screenshots for video generation stage
