...
async def main():
    ...
    input_image = ""
    if step_num == 1:
        base_imgs = sorted([f for f in os.listdir(IMAGE_DIR) if f.startswith("Generated_Image_")])
        if base_imgs:
            input_image = os.path.join(IMAGE_DIR, base_imgs[0])
        else:
            input_image = os.path.join(IMAGE_DIR, "Generated_Image_1.jpg")
    else:
